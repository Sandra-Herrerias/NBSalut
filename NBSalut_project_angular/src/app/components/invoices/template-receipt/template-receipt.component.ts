...
@Component({
  selector: 'app-template-receipt',
  templateUrl: './template-receipt.component.html',
  styleUrls: ['./template-receipt.component.css']
})
export class TemplateReceiptComponent implements OnInit {
  invoice: any | [];
  ...
  download() {
    ...
    html2canvas(this.element).then((canvas: any) => {
      ...
      doc.save('Recibo ' + this.invoice[0].invoice_number)
      this.active = true;
    })
  }
